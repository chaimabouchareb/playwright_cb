...
await page.locator('input[type="datetime-local"]').fill('2025-11-05T09:15');
...
await page.getByRole('searchbox', { name: 'Select an Animal Species' }).pressSequentially('Cow', { delay: 100 });
...
await page.getByRole('searchbox', { name: 'Select an Animal Type' }).pressSequentially('Dairy Cow', { delay: 100 });
await page.getByText('Dairy Cow').waitFor({ state: 'visible' });
await page.getByText('Dairy Cow', { exact: true }).click();
